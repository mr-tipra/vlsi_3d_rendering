...
extern "C" 
{
  void rendering( bit32 input[3*NUM_3D_TRI], bit32 output[NUM_FB])
  {
    ...
    // local variables
    Triangle_3D triangle_3ds;	//72 bits
    Triangle_2D triangle_2ds;	//56 bits
    Triangle_2D triangle_2ds_same;	//56 bits
#pragma HLS array_map variable=triangle_3ds instance=triangle horizontal
#pragma HLS array_map variable=triangle_2ds instance=triangle horizontal
#pragma HLS array_map variable=triangle_2ds_same instance=triangle horizontal
    bit16 size_fragment;
    CandidatePixel fragment[500];	//32 bits
#pragma HLS data_pack variable=fragment
    bit16 size_pixels;
    Pixel pixels[500];	//32 bits
#pragma HLS data_pack variable=pixels
    bit8 frame_buffer[MAX_X][MAX_Y];
//#pragma HLS ARRAY_RESHAPE variable=frame_buffer block factor=2 dim=1
    bit2 angle = 0;
  
    bit8 max_min[5];
    bit16 max_index[1];
#pragma HLS array_map variable=max_min instance=max_min_index horizontal
#pragma HLS array_map variable=max_index instance=max_min_index horizontal
    bit2 flag;
